...
export default function CreateListing() {
  ...
  const handleChange = (e) => {
    ...
    if(e.target.id === 'parkingLock' || e.target.id === 'tuned' || e.target.id === 'offer') {
      setFormData({
        ...formData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (+formData.regularPrice <= +formData.discountPrice) return setError('Discout price msut be lower than regular price');
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="flex gap-6 flex-wrap">
            ...
            <div className='flex gap-2'>
              <input type="checkbox" id="parkingLock" className="w-5" onChange={handleChange} checked={formData.parkingLock} />
              <span>Parking lock</span>
            </div>
            <div className='flex gap-2'>
              <input type="checkbox" id="tuned" className="w-5" onChange={handleChange} checked={formData.tuned} />
              <span>Tuned</span>
            </div>
            <div className='flex gap-2'>
          ...
          <div className='flex flex-wrap gap-6'>
            <div className='flex items-center gap-2'>
              <input type="number" id="wheels" min="1" max="10" required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.wheels}/>
              <p>Wheels</p>
            </div>
            <div className='flex items-center gap-2'>
              <input type="number" id="age" min="1" max="10" required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.age} />
              <p>Used-for (in months)</p>
            </div>
            <div className='flex items-center gap-2'>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          <div className='flex gap-4'>
            ...
            <button type='button' disabled={uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disableded:opacity-80'>{uploading ? 'Uploading...' : 'Upload'}</button>
          </div>
